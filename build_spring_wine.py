#!/usr/bin/env python3
"""Build spring wine guide generator."""
import os, json
from rich_article import rich_article, js_str, cover_svg

CONFIG_CODE = "require('./config')"
DATE = '20260614'

def build_article(name, title, digest, category, tags, content_blocks):
    svg_b64 = cover_svg('#2e7d32',
        ['春天喝什么葡萄酒？', '春暖花开'],
        '正是品酒好时节',
        '红樽坊 | 四季配酒')

    html = rich_article(content_blocks, primary='#2e7d32', secondary='#66bb6a')
    html_lines = html.split('\n')
    hlines = []
    for line in html_lines:
        escaped = js_str(line)
        if escaped:
            hlines.append(f"  '{escaped}' +")
    if hlines:
        hlines[-1] = hlines[-1].rstrip('+').rstrip()
    html_js = '\n'.join(hlines)

    gen_func = f'''function gen(){{
  return {html_js};
}}'''

    js = f'''const fs=require('fs'),path=require('path'),axios=require('axios'),FormData=require('form-data');

const config = {CONFIG_CODE};

const date={{full:'{DATE}'}};

function gCov(){{
  const svg="{js_str(svg_b64)}";
  return svg;
}}

{gen_func}

async function main(){{
  try{{
    const cb = await gCov();
    const art = {{
      title: '{js_str(title)}',
      author: '红樽坊',
      digest: '{js_str(digest)}',
      content: gen(),
      coverImage: '{name}_cover_ai.png',
      category: '{category}',
      tags: {json.dumps(tags, ensure_ascii=False)},
      publishDate: date.full
    }};
    fs.writeFileSync(
      path.join(__dirname, 'output', '{name}_'+date.full.replace(/-/g,'')+'.json'),
      JSON.stringify(art, null, 2)
    );

    const w = config.publish;
    const t = await axios.get('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='+w.appId+'&secret='+w.appSecret);
    const a = t.data.access_token;
    const sharp=require('sharp');
    const svgContent=Buffer.from(cb.split(',')[1],'base64').toString();
    const png=await sharp(Buffer.from(svgContent)).png().toBuffer();
    const f = new FormData();
    f.append('media', png, {{filename: 'cover.png', contentType: 'image/png'}});
    const m = await axios.post('https://api.weixin.qq.com/cgi-bin/material/add_material?access_token='+a+'&type=image', f, {{headers: f.getHeaders()}});
    const d = await axios.post('https://api.weixin.qq.com/cgi-bin/draft/add?access_token='+a, {{
      articles: [{{
        title: art.title,
        thumb_media_id: m.data.media_id,
        author: art.author,
        digest: art.digest,
        content: art.content,
        show_cover_pic: 1,
        need_open_comment: 0,
        only_fans_can_comment: 0
      }}]
    }});
    console.log('Spring, media_id:', d.data.media_id);
  }}catch(e){{
    console.error('Spring:', e.message);
    process.exit(1);
  }}
}}

main();
'''
    os.makedirs('output', exist_ok=True)
    with open(f'generate-{name}.js', 'w', encoding='utf-8') as f:
        f.write(js)
    print(f'Created generate-{name}.js')

if __name__ == '__main__':
    build_article('spring_wine',
        title='春天喝什么葡萄酒？春暖花开，正是品酒好时节',
        digest='春暖花开，万物复苏，正是品酒的好时节。从白葡萄酒到桃红，从轻盈红酒到起泡酒，这篇指南帮你选对春天的酒。',
        category='seasonal',
        tags=['春天','季节','春暖花开','清爽','复苏'],
        content_blocks=[
            {"type": "title", "text": "春天喝什么葡萄酒？"},
            {"type": "subtitle", "text": "春暖花开，正是品酒好时节"},
            {"type": "lead", "text": "春暖花开，万物复苏，正是品酒的好时节。冬天过去了，厚重的红酒可以暂时放一放，换上清爽的白葡萄酒、浪漫的桃红、轻盈的红酒。这篇指南帮你选对春天的酒。"},

            {"type": "h2", "text": "🌸 春天的特点"},
            {"type": "p", "text": "春天有这些特点，配酒要考虑："},
            {"type": "list", "items": [
                "<strong>温度回升</strong>——从寒冷到温暖，酒要清爽",
                "<strong>万物复苏</strong>——心情愉悦，酒要易饮",
                "<strong>户外活动多</strong>——野餐、踏青，酒要便携",
                "<strong>新鲜食材多</strong>——春笋、草莓、樱桃，要搭配",
                "<strong>聚会增多</strong>——朋友聚会、家庭聚餐，酒要百搭"
            ]},

            {"type": "h2", "text": "🍷 春天推荐酒款"},
            {"type": "p", "text": "根据春天的特点，推荐这些酒款："},

            {"type": "ri", "heading":"清爽担当：白葡萄酒", "text":"<strong>推荐酒款：</strong>长相思、雷司令、灰皮诺\n<strong>理由：</strong>白葡萄酒的清爽口感可以解春困，适合春天温暖的气候。\n<strong>饮用温度：</strong>8-10°C\n<strong>推荐搭配：</strong>春笋、海鲜、沙拉\n\n<strong>小贴士：</strong>白葡萄酒要冰镇，用冰桶保持低温。"},
            {"type": "ri", "heading":"浪漫担当：桃红（Rosé）", "text":"<strong>推荐酒款：</strong>普罗旺斯桃红、西班牙桃红\n<strong>理由：</strong>桃红的颜值超高，适合春天浪漫的氛围。\n<strong>饮用温度：</strong>8-10°C\n<strong>推荐搭配：</strong>草莓、樱桃、轻食\n\n<strong>小贴士：</strong>桃红颜色浪漫，适合发朋友圈。"},
            {"type": "ri", "heading":"轻盈担当：轻盈红酒", "text":"<strong>推荐酒款：</strong>黑皮诺、佳美、歌海娜\n<strong>理由：</strong>轻盈红酒的果味清新，适合春天温暖的气候。\n<strong>饮用温度：</strong>14-16°C\n<strong>推荐搭配：</strong>烤肉、春笋、奶酪\n\n<strong>小贴士：</strong>轻盈红酒要稍微冰镇，口感更好。"},
            {"type": "ri", "heading":"气氛担当：起泡酒（Sparkling）", "text":"<strong>推荐酒款：</strong>Prosecco、Cava、桃红起泡\n<strong>理由：</strong>起泡酒的气泡可以让人心情愉悦，适合春天热闹氛围。\n<strong>饮用温度：</strong>6-8°C\n<strong>推荐搭配：</strong>甜点、水果、轻食\n\n<strong>小贴士：</strong>开起泡酒时注意安全，不要对着人。"},

            {"type": "h2", "text": "🥬 春天配餐指南"},
            {"type": "p", "text": "春天有新鲜的食材，如何配酒？"},
            {"type": "table", "headers": ["食材", "推荐酒款", "理由"],
             "rows": [
                 ["春笋", "长相思、灰皮诺", "清爽酸度提升鲜味"],
                 ["草莓", "桃红、莫斯卡托", "甜味配草莓甜味"],
                 ["樱桃", "黑皮诺、佳美", "果味配樱桃果味"],
                 ["野菜", "白葡萄酒、桃红", "清爽解腻"],
                 ["烤肉", "轻盈红酒、桃红", "果味配烤肉香"]
             ]},

            {"type": "h2", "text": "🌿 春天饮酒场景"},
            {"type": "p", "text": "春天有不同场景，配酒也不同："},
            {"type": "list", "items": [
                "<strong>踏青野餐</strong>——冰镇白葡萄酒或桃红，清爽便携",
                "<strong>家庭聚餐</strong>——霞多丽或黑皮诺，百搭配餐",
                "<strong>朋友聚会</strong>——起泡酒或桃红，气氛活跃",
                "<strong>浪漫约会</strong>——香槟或桃红，浪漫氛围",
                "<strong>独自享受</strong>——雷司令或黑皮诺，放松心情"
            ]},

            {"type": "h2", "text": "📸 春天饮酒小贴士"},
            {"type": "p", "text": "记住这几个小贴士，让你的春天饮酒更完美："},
            {"type": "list", "items": [
                "<strong>选择清爽型</strong>——春天温度回升，酒要清爽",
                "<strong>注意温度</strong>——酒要冰镇，保持低温",
                "<strong>搭配新鲜食材</strong>——春笋、草莓、樱桃是绝配",
                "<strong>适量饮酒</strong>——微醺最好，喝醉伤身",
                "<strong>享受过程</strong>——最重要的是开心"
            ]},

            {"type": "h2", "text": "🚫 春天饮酒禁忌"},
            {"type": "p", "text": "春天饮酒，这些禁忌要注意："},
            {"type": "list", "items": [
                "<strong>不要喝太多</strong>——微醺最好，喝醉伤身",
                "<strong>不要喝太重</strong>——春天适合清爽型，不适合浓郁型",
                "<strong>不要忽略温度</strong>——酒要冰镇，保持低温",
                "<strong>不要忽略安全</strong>——喝酒后不要开车",
                "<strong>不要忽略健康</strong>——适量饮酒，有益健康"
            ]},

            {"type": "sep"},
            {"type": "quote", "text": "'春暖花开，正是品酒好时节。'"},
            {"type": "p", "text": "春天是万物复苏的季节，也是品酒的好时节。选对酒，享受春天的温暖和美好。"},

            {"type": "end", "text": "你春天喜欢喝什么酒？<br/>你有什么春天饮酒的经验？<br/>欢迎在评论区分享你的春天故事！"}
        ])
