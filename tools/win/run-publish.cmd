@echo off
rem ============================================================================
rem  Worker for the "publish pending articles" scheduled task.
...
rem
rem  Idempotent by design: only articles that have no draft yet are published.
rem  The FIRST run records a baseline instead of publishing every article, so a
rem  fresh install cannot flood the draft box (see tools/publish-pending.js).
rem
rem  Exit codes: 0 ok (published, or nothing to do) / 1 some failed / 127 no node
rem
...
set "LOG_FILE=%LOG_DIR%\publish.log"
...
if not exist "%PROJECT_DIR%\tools\publish-pending.js" (
  >>"%LOG_FILE%" echo [%DATE% %TIME%] ERROR: %PROJECT_DIR%\tools\publish-pending.js not found.
  endlocal & exit /b 127
)
...
rem log whose other content is UTF-8 from node. PowerShell gives a stable stamp.
set "TS="
...
>>"%LOG_FILE%" echo.
>>"%LOG_FILE%" echo ============================================================
>>"%LOG_FILE%" echo [%TS%] publish started  (node: %NODE_EXE%)

rem No --quiet here: the tool's own output (article -> draft media_id) is
rem exactly what the log should record for later auditing.
"%NODE_EXE%" "tools\publish-pending.js" >>"%LOG_FILE%" 2>&1
set "RC=%ERRORLEVEL%"

>>"%LOG_FILE%" echo [%TS%] finished, exit code %RC%
if "%RC%"=="0" >>"%LOG_FILE%" echo status: OK (published, or nothing to do)
if "%RC%"=="1" >>"%LOG_FILE%" echo status: SOME FAILED - see details above
if "%RC%"=="2" >>"%LOG_FILE%" echo status: ENVIRONMENT ERROR - bad arguments or missing .env
